...
object TranspilerUtils:
  /** Represents a test case for the transpiler
    *
    * @param name
    *   The name of the test, with the number of points as (Xpts) or (Xpt)
    * @param input
    *   The lines of the input file, i.e., the alpine code
    * @param expected
    *   The expected lines of the std out when running the transpiled program
    */
  case class TranspilerTest(
      name: String,
      input: List[String],
      expected: List[String]
  )

  /** Parses a file with the correct format and produces a list of
    * TranspilerTest instances
    *
    * @param lines
    * @return
    */
  def parseTests(lines: List[String]): List[TranspilerTest] =
    val indicesBeginningTests =
      lines.zipWithIndex.filter(p => p._1.startsWith("//BEGIN")).map(_._2)
    val indicesEndTests =
      lines.zipWithIndex.filter(p => p._1.startsWith("//END")).map(_._2 + 1)
    ...
    testLists.map(l =>
      ...
      val out = l
        .slice(
          l.indexWhere(_.startsWith("//OUT")),
          l.indexWhere(_.startsWith("//END"))
        )
  ...
  class Runner:
    ...
    case class ScalacCompileError(file: String, message: String)
        extends Exception(message):
      override def toString: String =
        f"Failed to compile the Scala file $file: \n$message"
    ...
    private val scalac =
      if System.getProperty("os.name").startsWith("Windows") then
        "where.exe scalac".!!
      else "scalac"
    private val scala =
      if System.getProperty("os.name").startsWith("Windows") then
    ...
    private def spawn(
        cmd: String,
        cwd: Option[File] = Some(tmpDir.toFile),
        ignoreStderr: Boolean = false
    ): (Int, String, String) =
    ...
    def run(input: Path): Either[ScalaRunError, String] =
      ...
      val (exitCode, output, stderr) =
        spawn(f"$scala $options $absolutePaths", ignoreStderr = true)
    ...
    /** Run the alpine transpiler, returning the Path to the generated .scala
      * file
      *
      * @param inputFile
      * @return
    ...
    def runAlpineTranspiler(inputFile: Path): Either[BackendError, Path] =
      try
        val source =
          SourceFile.withContentsOfFile(inputFile.toAbsolutePath.toString).get
      ...
      catch (e: Throwable) => Left(BackendError(e))

    /** Compiles the given Scala files in the temporary directory */
    def compileLibrary(inputs: List[String]): Unit =
      val absolutePaths = inputs
        .map(filename => tmpDir.resolve(appendScalaExtension(filename)))
        .mkString(" ")
      spawn(f"$scalac $absolutePaths", Some(tmpDir.toFile)) match
        case (0, _, _) => ()
        case (_, stdout, stderr) =>
          throw ScalacCompileError(
            "scala_rt",
            stderr ++ "\n-- stderr --\n" ++ stdout
          )

    /** Writes a Scala file in the temporary directory. Prepends the .scala if
      * needed extension
      */
    ...
    /** Writes an Alpine file in the temporary directory. Prepends the .al if
      * needed extension
      */
