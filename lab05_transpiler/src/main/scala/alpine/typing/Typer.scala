...
final class Typer(
  ...
  def visitConditional(e: ast.Conditional)(using context: Typer.Context): Type =
    ...
    if succType == failType then context.obligations.constrain(e, succType)
    else
      ...
      context.obligations.add(
        Constraint.Subtype(
          succType,
          freshTypeVar,
          Constraint.Origin(e.successCase.site)
        )
      )
      context.obligations.add(
        Constraint.Subtype(
          failType,
          freshTypeVar,
          Constraint.Origin(e.failureCase.site)
        )
      )
  ...
  def visitLet(e: ast.Let)(using context: Typer.Context): Type =
    ...
    e.binding.visit(this) match
      ...
      case t =>
        val bodyType =
          context.inScope(e, (inner) => e.body.visit(this)(using inner))
        context.obligations.add(
  ...
  def visitRecordType(e: ast.RecordType)(using context: Typer.Context): Type =
    val labeledTypes =
      e.fields.map(f => Type.Labeled(f.label, f.value.visit(this)))
  ...
  private def commit(
    ...
    for (n, r) <- (obligations.inferredBinding ++ solution.binding) do
      val s = r.withTypeTransformed((t) =>
        solution.substitution.reify(t)
      ) // ← This line changes.
